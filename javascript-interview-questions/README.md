...
注意这里，```b```实际上是一个全局变量，所以b的作用范围明显能在任何地方被访问到（如果没有被子作用域覆盖掉的话）。所以输出的结果，第一行是```false```，第二行是```true```。
...
### 11.写一个返回boolean值得少于80字符的函数来判断一个字符串是否是回文。
...
上面代码中```plus```嵌套了两个箭头函数，```x```依然可以在嘴里层的函数中被访问（```x + y```），并且外层函数中返回了```y => x + y```。
...
第一个为```true```，第二个为```false```。```==```对比时进行类型转换，```===```对比时严格进行类型和值得对比。
...
### 25.创建一个函数，对于给定的页面中的DOM节点，访问检点本身及其子孙节点。对于每个访问的节点，函数都应该将其作为参数传递给一个回调函数。函数的参数如下：1. 一个DOM节点；2. 一个回调函数。
